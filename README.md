...
The ```src``` folder here in this repo is generated by typing ```yo aspnet``` and choosing the F# web project option.  
...
#### Note: if you've checked out the code sample from this repo, you'll need to switch to the "src" directory and also run ```bower install``` to restore the JavaScript and other libs. 
...
```docker-machine ip``` will give you this IP.  On Windows, it's in Hyper-V so you don't need to worry - it's running on localhost. 
